...
## Mechanisms of plant-plant facilitation globally.
...
Mech <- colSums(dat2)
```
### environments where positive plant interactions are found
...
### number of studies examining nurse mechanism and hypothesis
``` {r, nurse mechanism, echo=FALSE}
...
### pairwise comparisons of nurse hypothesis and mechanism
...
fit1 <- aov(studies~nurse, data=nursetest)

fit2 <- aov(studies~nurse, data=concepttest)

fit3 <- kruskal.test(studies~nurse, data=concepttest)
library(pgirmess)
kruskalmc(studies~nurse + concept, data=concepttest)

fit3 <- glm(studies~nurse, data=concepttest)
