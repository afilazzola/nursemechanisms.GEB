...
## Create a higher resolutino of map

require(ggmap)
require(maps)
###  Start with base map of world
mp <- NULL
mapWorld <- borders("world", colour="white", fill="orange") # create a layer of borders
mp <- ggplot() + theme_bw()+  mapWorld

mp <- mp+ geom_point(data=locations , aes(x=lon, y=lat), size=2, pch=21, fill="Red") + scale_fill_manual(values=c("#D55E00", "#009E73", "#56B4E9","grey50")) + ylab("Latitude") + xlab("Longitude") 
mp
```
...
xlab1 <- c("Direct Interactions","Biotic Stress","Evolution","Indirect Interactions","Invasion","Population Dynamics","Restoration","Abiotic Stress","Succession")
...
xlab2 <- c("Stress Amelioration","Distribution","Evolution","Herbivory","Pollinator","Seeds","Soil Moisture","Soil Nutrient","Substrate")
...
## Revise figure output
lbls <- c("Tropical", "Temperate", "Arctic-Alpine","Arid and Semi-Arid","Mediterranean","Other")
enviro <- c(15.1,8.2,18.6,43.6,11.7,2.7)

pieData <- data.frame(Ecosystem = lbls, Studies = enviro, colors=c("#fe0000","#008001", "#56B4E9", "#E69F00","#d1b48c","#81007f"), stringsAsFactors = F) %>% arrange(Ecosystem)

pie <- ggplot(pieData, aes(x="", y=Studies, fill=Ecosystem))+ geom_bar(width = 1, stat = "identity")+ coord_polar("y", start=0)
pie <- pie + theme_classic() + scale_fill_manual(values=pieData$colors)
pie 
```
...
plot2 <- barplot(dat2, xlab="", xaxt="n", ylab="Number of Articles", cex.lab=1.4, cex.names=1.2, ylim=c(0,120))
axis(side=1, at=plot2, labels=FALSE)
text(plot2,par("usr")[3] - 3, srt = 35, adj = 1,labels=xlab2,xpd = TRUE)

plot1 <- barplot(dat1, xlab="", xaxt='n', ylim=c(0,120),  cex.lab=1.4, cex.names=1.2, legend.text=TRUE, args.legend= list(x=12, y=110, cex=0.8, legend=legend))
axis(side=1, at=plot1, labels=FALSE)
text(plot1,par("usr")[3] - 3, srt = 35, adj = 1,labels=xlab1,xpd = TRUE)
